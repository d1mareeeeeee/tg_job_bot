# CLAUDE.md — контекст проекта для Claude Code

Телеграм-бот подбора вакансий: читает посты из публичных Telegram-каналов,
отсеивает не-вакансии, оценивает остальные через LLM на соответствие резюме
(0–100) и присылает подходящие в личку через бота-уведомителя. Работает по
расписанию, один пост дважды не обрабатывает.

## Архитектура
- `main.py` — оркестрация + планировщик (asyncio-цикл, graceful shutdown).
- `config.py` — чтение/валидация `.env` в dataclass `Config`.
- `telegram_reader.py` — `WebReader`: чтение каналов через веб-превью `t.me/s/`.
- `prefilter.py` — отсев явного мусора перед LLM (чёрный список, см. ниже).
- `llm_client.py` — абстракция LLM (OpenAI/Anthropic/Gemini) + `PROMPT_TEMPLATE`.
- `notifier.py` — отправка уведомлений через Telegram Bot API (httpx).
- `db.py` — SQLite: `seen_posts`, `matches`.
- `dev_tests/` — ручные проверки (не pytest, запускаются напрямую): `test_seen_logic.py`
  — 39 проверок `run_cycle`/`main_loop` на заглушках, без сети; `test_live_error.py`
  — живая проверка доставки ошибок, **шлёт реальные сообщения** (см. ниже).
- `resume.txt` / `positions.txt` — критерии оценки (см. `RESUME_PATH`); реальные
  файлы gitignored, в репозитории только шаблоны `*.example`.

## Ключевые решения (важно не сломать)
- **Чтение без api-ключа Telegram.** Каналы читаются веб-парсером
  `https://t.me/s/<username>` (httpx + BeautifulSoup), НЕ через Telethon/MTProto.
  Следствие: работают только **публичные** каналы с веб-превью; приватные и
  инвайт-ссылки (`t.me/+…`, `joinchat/…`) не поддерживаются и пропускаются.
- **LLM через OpenRouter (OpenAI-совместимый режим).** `LLM_PROVIDER=openai` +
  `LLM_BASE_URL=https://openrouter.ai/api/v1`, ключ `sk-or-…`. Модель — в формате
  OpenRouter (`провайдер/модель`, у бесплатных суффикс `:free`).
- **`LLM_MODEL` обязателен** — дефолт не подставляется (иначе `ConfigError`).
- **Для бесплатных моделей `LLM_JSON_MODE=false`** — многие не поддерживают
  `response_format` и падают. Парсер JSON в `llm_client._parse_json` устойчив и
  без строгого режима.
- **`LLM_REQUIRE_FREE=true`** — при старте проверяет через OpenRouter `/models`,
  что модель бесплатна; платная → `ConfigError` и выход (`verify_free_model`).
- **Префильтр — чёрный список, а не белый.** В LLM уходит ВСЁ, кроме явного мусора
  (короткие тексты, резюме кандидатов `#резюме`, инфоцыганство `AD_MARKERS`, реклама
  `SPAM_MARKERS` без сильного маркера вакансии). Раньше было наоборот — пост проходил,
  только если похож на вакансию, — и это теряло вакансии: форматы объявлений слишком
  разнообразны, чтобы их перечислить, а мусор однотипен. Ошибки несимметричны:
  потерянная вакансия необратима (пост помечен `seen`), лишний пост стоит одного
  вызова LLM и отсеется порогом. Спорное (стажировки, дубли закреплённых сообщений)
  сознательно отдаём в LLM. Замер на 605 размеченных постах: recall 100%,
  precision 98.3% (было 92.6% / 96.0%). **Не «оптимизировать» обратно в белый список.**
- **Первый цикл после запуска читает канал шире** (`STARTUP_LOOKBACK_MESSAGES`, 100
  против обычных 20). Окно привязано к последним N постам и движется только вперёд:
  то, что в него не попало, не рассматривается никогда. При работе не круглосуточно
  за перерыв выходит больше постов, чем помещается в обычное окно. Замер: за 12 ч
  максимум 19 постов (запас в один пост при окне 20), за сутки — 32. Окно сужается
  только после **успешного** цикла, где успешный = цикл не упал **и** прочитал хотя бы
  один канал (`CycleResult.channels_unreadable`): если он упал или сеть не пустила к
  t.me, посты не помечены seen и на повторе широкое окно ещё нужно. Раньше проверялось
  только «не упал», и цикл с полностью недоступными каналами съедал широкое окно —
  ровно в том случае, для которого оно и заведено. Реализация — `replace` в `main_loop`.
  По той же причине не снимается `first_run_silent`: тихий проход при недоступных
  каналах ничего не помечает seen, и переход в обычный режим прислал бы все старые
  вакансии.
- **Полная недоступность каналов — событие для пользователя.** Ошибки чтения отдельных
  каналов в личку не шлём (шумно), но «не прочитан ни один канал» (`t.me` недоступен:
  сеть, файрвол, выключенный VPN) уходит в бота один раз на инцидент, и один раз —
  сообщение о восстановлении (флаг `unreadable_notified` в `main_loop`, дедупликация как
  у `last_crash`). Сообщение обычно доходит: блокируется `t.me`, а `api.telegram.org`
  работает. У сетевых исключений httpx текст часто пустой — в логе всегда пишем класс
  исключения (`telegram_reader._describe`), иначе получается «Сетевая ошибка: » без причины.
- **Подборки вакансий оцениваются по лучшей позиции** (`PROMPT_TEMPLATE`). Каналы
  часто публикуют несколько вакансий одним постом («DS (CV), MLE (audio), DS (LLM)»).
  LLM оценивает каждую и возвращает оценку самой подходящей, указывая её в `role`.
  Поэтому личные ограничения (нежелательные направления, грейд) держим в
  `positions.txt`, а НЕ в префильтре: он отсёк бы весь пост целиком вместе с
  подходящей вакансией.
- **Жёсткое правило в промпте:** если формат вакансии — гибрид/очно и полная
  удалёнка невозможна, LLM возвращает `score=0` (см. `PROMPT_TEMPLATE`).
- **Персонализация размазана по коду, не только по `.env`:** `PROMPT_TEMPLATE`
  (правило про удалёнку, шкала, грейд, язык ответа), списки `prefilter.py`
  (русскоязычные, настроены на IT/DS-каналы), `notifier._build_message` (формат
  сообщения). Всё это описано в README, раздел «Персонализация: не только `.env`» —
  при правках этих мест синхронизировать раздел, иначе чужой пользователь с офисным
  поиском получит нули по всем вакансиям и не поймёт почему.
- **Формат уведомления:** только оценка + текст вакансии без изменений + ссылка
  (`notifier._build_message`); обрезка только на лимите Telegram 4096.
- **`seen` ставится только после реальной обработки поста** (не до, как раньше).
  `score_vacancy` больше не возвращает молчаливый `score=0` при сбое, а бросает
  `LLMCallError` (провайдер не ответил) или `LLMParseError` (ответ не разобран):
  - `LLMCallError` → пост НЕ помечается seen, вернётся в следующем цикле;
    `LLM_FAILURE_STREAK` (3) сбоев подряд → цикл прерывается и бот спит
    `LLM_BACKOFF_MINUTES` (30) вместо обычного интервала;
  - `LLMParseError` → пост помечается seen и уходит в бота с пометкой
    «⚠️ ОШИБКА ПАРСИНГА» **мимо порога** — вакансия не теряется.
  Не возвращать здесь «безопасный» `score=0`: он неотличим от честной оценки 0.
- **Об ошибках бот пишет в личку** (`notifier.send_error`, поверх общего
  `_send_text`): каждый `LLMCallError` — с номером попытки «N из 3», ссылкой на пост
  и текстом ошибки (не более 3 сообщений за цикл, чтобы не флудить); непредвиденное
  падение цикла — один раз на сигнатуру ошибки (повтор той же ошибки в следующем
  цикле не дублируется). Ошибки чтения каналов НЕ шлём — слишком шумно.

## Запуск
Из папки проекта, интерпретатор из venv:
```bash
.venv/bin/python main.py --init   # тихий первый проход (пометить старые посты seen)
.venv/bin/python main.py          # обычная постоянная работа
```
`main.py --init` НЕ завершается: после тихого прохода переходит в обычный режим и
работает дальше в том же процессе. Остановка — Ctrl+C.

## Готчи
- **venv:** зависимости ставить в `.venv` (`.venv/bin/pip install -r requirements.txt`),
  не в системный Python. Запуск — `.venv/bin/python …`.
- **Код и `.env` читаются один раз при старте** — после правок нужен перезапуск.
- **Лимиты free-моделей OpenRouter:** суточный кап запросов; при `429` увеличить
  `LLM_RATE_DELAY` / уменьшить `LOOKBACK_MESSAGES` / пополнить баланс.
- **Каналы `@it_jobs` и подобные** могут не читаться (нет веб-превью) — проверять
  через `https://t.me/s/<username>` в браузере.

## Обязательные переменные `.env`
`CHANNELS`, `NOTIFY_BOT_TOKEN`, `NOTIFY_CHAT_ID`, `LLM_API_KEY`, `LLM_MODEL`
(+ `LLM_BASE_URL` для OpenRouter). Остальное — с дефолтами (см. `.env.example`).

## Git / приватность (проект готовится к публикации на GitHub)
- **Не коммитить** (в `.gitignore`): `.env` и все варианты `.env.*` (кроме
  `.env.example`), `resume.txt`, `positions.txt`, `.venv*/`, `job_bot.sqlite3*`,
  `job_bot.log`, `.claude/` целиком, а также превентивно ключи и сертификаты
  (`*.pem`, `*.key`, `*.p12`, `id_rsa*`, `*credentials*`, `*secrets*`, `*.sql`).
  Игнорируется именно папка `.claude/`, а не один файл в ней: иначе любой новый
  файл внутри попадёт в коммит вместе с машинно-зависимыми путями.
- **Токен бота маскируется в логах** (`notifier._redact`): он вшит в URL Bot API,
  поэтому теоретически может просочиться в текст исключения или ответа сервера.
  Логи часто прикладывают к issue — не убирать эту страховку.
- **Коммитить** только шаблоны: `.env.example`, `resume.txt.example`,
  `positions.txt.example` — в них ТОЛЬКО плейсхолдеры, без реальных значений.
- В `README.md`, `*.example` и `dev_tests/` не должно быть реальных данных: chat_id,
  токена бота, ключа OpenRouter, реальных имён каналов. Использовать placeholder'ы
  (в тестах — `testch` / `testchannel`).
- `dev_tests/` — коммитить (заглушки и плейсхолдеры, секретов нет). `test_live_error.py`
  берёт токен из `.env` и **шлёт реальные сообщения в личку**: без флага не делает
  ничего, запускать только осознанно (`--call` / `--parse` / `--crash` / `--all`).
- `CLAUDE.md` — коммитить (общий контекст, без секретов). Обновлять вручную после
  значимых изменений поведения бота.
